...
class OffboardControl(DroneConnection):
    ...
    async def end_mission(self) -> None:
        """
        End mission with proper cleanup sequence:
        1. Stop telemetry tasks
        2. Stop offboard mode (safely exit offboard control)
        3. Land drone
        4. Wait for landing completion
        5. Disarm drone
        
        This method ensures clean transition from offboard to manual control.
        """
        print("🔄 Ending mission with proper cleanup sequence...")
        await asyncio.sleep(1)
        
        # Step 1: Stop all telemetry tasks
        ...
        # Step 2: Stop offboard mode (safely exit offboard control)
        print("🎮 Stopping offboard control...")
        try:
            await self.drone.offboard.stop()
            print("✅ Offboard control stopped - safe to land")
        ...
        while True:
            try:
                current_alt = self.current_position.absolute_altitude_m
                if current_alt < 0.5:  # Consider landed when altitude is less than 0.5m
                    print("✅ Landing completed. Disarming drone...")
                    break
                await asyncio.sleep(1)
            except:
                # If we can't get altitude, wait a bit more
                print("⚠️ Could not get altitude, waiting for landing...")
                await asyncio.sleep(2)
                break
        ...
        try:
            ...
            print(f"⚠️ Could not disarm drone: {e}")
        
        print("🎯 Mission completely finished - drone landed and disarmed")
        print("   📍 Safe for maintenance and next mission")
